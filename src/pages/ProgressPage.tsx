...
export function ProgressPage() {
  ...
  const observedLoss = first && last ? (first.weight - last.weight) * 7 / spanDays : 0
  const suggestion = suggestedCalorieAdjustment(observedLoss, state.profile.weeklyLossTargetKg)
  const currentAverage = averaged.at(-1)?.average ?? last?.weight ?? state.profile.currentWeightKg
  const loss = state.profile.referenceWeightKg - currentAverage
  const trajectoryData = averaged.map((point, index) => ({ label: formatCompactDate(point.date), weight: point.weight, average: point.average, target: state.profile.referenceWeightKg - state.profile.weeklyLossTargetKg * index / 7 }))
  ...
  const kgRemaining = Math.max(0, currentAverage - state.profile.targetWeightKg)
  const daysRemaining = Math.round(kgRemaining / Math.max(.1, state.profile.weeklyLossTargetKg) * 7)
    ...
    <div className="reference-page progress-reference">
      ...
      <div className="reference-body progress-body">
        <section className="progress-kpis-reference">
          ...
          <ProgressKpi icon={<TrendingDown />} label="Perte totale" value={-loss} unit="kg" signed tone="teal" help="Différence entre le poids de référence et la moyenne de poids la plus récente." />
          <ProgressKpi icon={<Gauge />} label="Rythme actuel" value={-observedLoss} unit="kg / semaine" signed tone="teal" help="Vitesse de variation extrapolée sur une semaine à partir des pesées disponibles." placement="right" />
        </section>

        <section className="progress-main-reference">
          <div className="weight-trajectory-card reference-card"><h2><ExplainedLabel help="Compare chaque pesée, la moyenne mobile et la pente théorique correspondant à ton objectif hebdomadaire." placement="left">Poids réel vs trajectoire cible</ExplainedLabel></h2><div className="chart-legend-reference"><span className="dot-legend"/>Poids quotidien <span className="line-teal"/>Moyenne 7 jours <span className="line-navy"/>Trajectoire −{formatDecimal(state.profile.weeklyLossTargetKg)} kg / sem.</div><div className="trajectory-chart"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={trajectoryData} margin={{ top: 18, right: 18, left: 0, bottom: 8 }}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#dfe6ec"/><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fontSize: 10 }}/><YAxis width={46} domain={['dataMin - 1', 'dataMax + 1']} axisLine={false} tickLine={false} tickFormatter={(value) => formatDecimal(Number(value), 1)}/><Tooltip formatter={(value) => `${formatDecimal(Number(value), 2)} kg`}/><Area dataKey="target" fill="#eef1f4" stroke="none" isAnimationActive={false}/><Scatter dataKey="weight" fill="#0a9089" isAnimationActive={false}/><Line dataKey="average" type="monotone" stroke="#0a9089" strokeWidth={4} dot={false} connectNulls isAnimationActive={false}/><Line dataKey="target" type="monotone" stroke="#071a42" strokeWidth={3} dot={false} isAnimationActive={false}/><ReferenceLine y={state.profile.targetWeightKg} stroke="#0a9089" strokeDasharray="4 4"/></ComposedChart></ResponsiveContainer></div></div>
          <aside className="progress-diagnostic-column">
            <section className="diagnostic-reference-card reference-card"><h2><ExplainedLabel help="Analyse l’écart entre le rythme de perte observé et le rythme demandé. Une recommandation n’est proposée qu’à partir des données enregistrées." placement="right">Diagnostic automatique</ExplainedLabel></h2><h3><CheckCircle2 /> {Math.abs(observedLoss - state.profile.weeklyLossTargetKg) <= .2 ? 'Rythme dans la cible' : 'Rythme à surveiller'}</h3><p>Zone cible : −{formatDecimal(state.profile.weeklyLossTargetKg - .2)} à −{formatDecimal(state.profile.weeklyLossTargetKg + .1)} kg / semaine</p><div className="diagnostic-scale"><span style={{ left: `${Math.min(95, Math.max(5, observedLoss / 1.2 * 100))}%` }}/></div><strong className="diagnostic-number">−{formatDecimal(observedLoss, 2)}</strong><button onClick={applySuggestion}><ThumbsUp /> {suggestion === 0 ? 'Conserver les calories actuelles pendant 7 jours' : `${suggestion > 0 ? 'Ajouter' : 'Retirer'} ${Math.abs(suggestion)} kcal après confirmation`}</button></section>
